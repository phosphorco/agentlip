...
| Auth token is stored with mode 0600 (and .agentlip dir 0700) | ✅ | `packages/hub/src/securityBaseline.test.ts` → `server.json security` suite (`writeServerJson` creates 0600; `.agentlip/` dir 0700). |
...
| Plugin isolation: plugins cannot write to `.agentlip/` | ⏸️ deferred | Requires Phase 3 plugin runtime/isolation (`bd-16d.4.*`). Will be enforced + tested when plugin execution exists. |
| Workspace config loaded only from discovered workspace root (no upward traversal surprises) | ✅ | `packages/workspace/src/index.test.ts` (workspace discovery + boundary behavior + `.agentlip` perms). `packages/cli/src/index.test.ts` (CLI workspace discovery + read-only open). |
...
| server.json permissions (0600) + .agentlip dir permissions (0700) | ✅ | `packages/hub/src/securityBaseline.test.ts` (`writeServerJson` + dir perms) |
...
Deferred because the plugin runtime/isolation does not exist yet in Phase 2. Once Phase 3 lands, add a test that runs an actual plugin (Worker/subprocess) and asserts it cannot write under `.agentlip/`.
