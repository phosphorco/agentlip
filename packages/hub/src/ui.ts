 ...
 * 
 * Supports two modes (controlled by HUB_UI_SPA_ENABLED env var):
 * - SPA mode (default): serves bootstrap endpoint, static assets, and SPA fallback
 * - Legacy mode: inline HTML/CSS/JS pages (for rollback safety)
 * 
 * Security: all user content escaped via textContent, URLs validated.
 */

import { UI_ASSETS, UI_ENTRY_HTML, type UiAsset } from "./uiAssets.generated";
...
/**
 * Determine if SPA mode is enabled.
 * Defaults to true (SPA enabled) unless explicitly set to "false" or "0".
 */
function isSpaEnabled(): boolean {
  const envValue = process.env.HUB_UI_SPA_ENABLED;
...
export function handleUiRequest(req: Request, ctx: UiContext): Response | null {
  ...
  if (spaEnabled) {
    ...
    // No match in SPA mode
    return null;
  }

  // ─── Legacy Mode Routes ──────────────────────────────────────────────────

  // GET /ui/bootstrap → 404 in legacy mode
  if (path === "/ui/bootstrap") {
    return new Response("Not Found", { status: 404 });
  }

  // GET /ui/assets/* → 404 in legacy mode
  if (path.startsWith("/ui/assets/")) {
    return new Response("Not Found", { status: 404 });
  }
